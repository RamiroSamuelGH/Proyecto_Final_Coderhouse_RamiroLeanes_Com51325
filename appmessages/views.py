...
def enviarMensaje(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "inicioMensajeria.html", {"mensaje":f"Mensaje para {mensaje.receptor} enviado correctamente", "avatar":obtenerAvatar(request)})
        else:
            return render(request, "enviarMensaje.html", {"form": form, "mensaje":"Datos no validos, ingrese otra vez", "avatar":obtenerAvatar(request)})
...
def verMensajes(request): 
    ...
    return render(request, "bandejaDeEntrada.html", {"emailreceptor" : emailreceptor, "avatar":obtenerAvatar(request)})
